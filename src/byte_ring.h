...
// Producer writes variable-length records; consumer drains to spool/block builder.
class ByteRing {
public:
  ByteRing() : buf_(nullptr), cap_(0), head_(0), tail_(0), drops_(0) {}
  ...
  uint32_t peek(void* dst, uint32_t n) const {
    uint32_t avail = available();
    if (n > avail) n = avail;
    uint8_t* p = (uint8_t*)dst;
    uint32_t t = tail_;
